...
export const DepartmentTeachersPageContent = () => {
    ...
    if(isDepartmentTeachersLoading) return <div className="PageAdmin"><Loader/></div>
    ...
    return <div className="PageAdmin">
         <Card resetFilters={()=>'test'} title="Ученики" valueFirst={`Всего учителей ${departmentTeachers?.meta.total_students}`}>
                           {departmentTeachers&&departmentTeachers.data&&departmentTeachers.data.length>0 ? <Table<IDepartmentStudent,any> data={departmentTeachers.data} getColumns={()=>getDepartmentAllStudentsColumns()}/> : "Данных нет"}
                       </Card>
    </div>
}

...
export const DepartmentStudentsPageContent = () => {
    ...
    if(isDepartmentStudentsLoading) return <div className="PageAdmin"><Loader/></div>
    ...
    return <div className="PageAdmin">
         <Card resetFilters={()=>'test'} title="Ученики" valueFirst={`Всего учеников ${departmentStudents?.meta.total_students}`}>
                           {departmentStudents&&departmentStudents.data&&departmentStudents.data.length>0 ? <Table<IDepartmentStudent,any> data={departmentStudents.data} getColumns={()=>getDepartmentAllStudentsColumns()}/> : "Данных нет"}
                       </Card>
    </div>
}

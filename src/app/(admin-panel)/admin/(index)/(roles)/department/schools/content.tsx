...
export const DepartmentSchoolsAdminPageContent = ()=> {
  ...
  const {departmentSchools,isDepartmentSchoolsError,isDepartmentSchoolsLoading} = useGetSchools()
     if(isDepartmentSchoolsLoading) return <div><Loader/></div>
    ...
    return <div className="PageAdmin">
         <Card resetFilters={()=>'test'} title="Школы" valueFirst={`Всего школ ${departmentSchools?.meta.total_schools}`}>
                   {departmentSchools&&departmentSchools.data&&departmentSchools.data.length>0 ? <Table<IMinistrySchool,any> data={departmentSchools.data} handleRowClick={handleItemClick} getColumns={()=>getSchoolsColumns()}/> : "Данных нет"}
               </Card>
    </div>
}

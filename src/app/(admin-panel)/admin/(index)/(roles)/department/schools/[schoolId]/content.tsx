            ...
            <div className="UniqueSchoolPageContent__main">
                <div className="UniqueSchoolPageContent__cards">
                    <ResultsCard
                        variant="admin"
                        title="Учеников"
                        value={departmentSchool?.data.statistics.total_students}
                    />
                    <ResultsCard
                        variant="admin"
                        title="Учителей"
                        value={departmentSchool?.data.statistics.total_teachers}
                    />
                    <ResultsCard
                        variant="admin"
                        title="Классы"
                        value={departmentSchool?.data.statistics.total_classes}
                    />
                    <ResultsCard
                        variant="admin"
                        title="Баллы"
                        value={departmentSchool?.data.statistics.total_points}
                    />
                    <ResultsCard
                        variant="admin"
                        title="Ср. балл"
                        value={departmentSchool?.data.statistics.average_points}
                    />
                </div>
            </div>
            <ClassCardMain title="Лучшие ученики школы" linkHref="/" linkText="Полный список">
                {departmentSchool && departmentSchool.data && departmentSchool.data.top_students.length > 0 ? (
                    <Table<SchoolTopStudent, any>
                        data={departmentSchool.data.top_students}

...
export const UniqueDistrictPageContent = () => {
	...
	const { isMinistryDistrictLoading, ministryDistrict, isMinistryDistrictError } = useGetDistrictById(Number(districtId))
		...
		<div className="AdminPage">
			...
			<div>
				<ClassCardMain title="Лучшие школы" linkHref="/" linkText="Полный список">
					{ministryDistrict && ministryDistrict.data && ministryDistrict.data.top_schools.length > 0 ? (
						<Table<UniqueDistrictTopSchool> data={ministryDistrict.data.top_schools} getColumns={() => getTopSchoolsColumns()} />
					) : (
						'Данных нет'
					)}
				</ClassCardMain>
				<ClassCardMain title="Лучшие ученики" linkHref="/" linkText="Полный список">
					{ministryDistrict && ministryDistrict.data && ministryDistrict.data.top_schools.length > 0 ? (
						<Table<UniqueDistrictTopStudent> data={ministryDistrict.data.top_students} getColumns={() => getTopStudentsColumns()} />
					) : (
						'Данных нет'
					)}
				</ClassCardMain>

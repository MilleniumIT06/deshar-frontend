...
export const ChartCandle = ({ maxPoints, currentPoints, date = new Date() }: ChartCandleProps) => {
	const safeMaxPoints = Math.max(1, maxPoints)
	const clampedPoints = Math.max(0, Math.min(currentPoints, maxPoints))

	const pointsPercent = Math.min(100, Math.max(0, (clampedPoints / safeMaxPoints) * 100))

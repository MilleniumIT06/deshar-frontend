...
export const DepartmentManagerView = () => {
	...
	const transformedSchoolsStats =
		districtStats?.data.top_schools
			.map((item, index) => ({
				name: item.name,
	...
	return (
		<div className="CommonView">
			...
			<div className="CommonView__cards CommonView__cards--department">
				{isDistrictStatsLoading ? (
					'statistic loading...'
				) : (
					<ResultsCard variant="admin" value={districtStats?.data.statistics.total_points} title="Баллы" />
				)}
				{isDistrictStatsLoading ? (
					'statistic loading...'
				) : (
					<ResultsCard variant="admin" value={districtStats?.data.statistics.average_points} title="Ср. Балл" />
				)}
				{isDistrictStatsLoading ? (
					'statistic loading...'
				) : (
					<ResultsCard variant="admin" value={districtStats?.data.statistics.total_schools} title="Всего школ" />
				)}
				{isDistrictStatsLoading ? (
					'statistic loading...'
				) : (
					<ResultsCard variant="admin" value={districtStats?.data.statistics.total_students} title="Всего учеников" />
				)}
				{isDistrictStatsLoading ? (
					'statistic loading...'
				) : (
					<ResultsCard variant="admin" value={districtStats?.data.statistics.total_teachers} title="Всего учителей" />
				)}
			</div>
			<div>
				{isDistrictStatsLoading ? (
					...
					<ClassCardMain title={'Лучшие ученики среди школ'} linkText="Полный список" linkHref="/">

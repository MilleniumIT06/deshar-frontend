...
class StatsService {
	...
	async getSchoolAllStudents() {
		const { data } = await axiosWithAuth<{
			data: ISchoolAllStudents[]
			meta: { school_id: Id; school_name: string; total: number }
		}>({
	...
	async getDepartmentSchools() {
		const { data } = await axiosWithAuth<{
			data: IMinistrySchool[]
			meta: { district_id: Id; total_schools: number }
		}>({
	...
	async getDepartmentStudents() {
		const { data } = await axiosWithAuth<{
			...
			meta: { district_id: Id; total_students: number }
		}>({
